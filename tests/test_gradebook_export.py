...
def test_extract_identity_still_reads_legacy_date_label() -> None:
    identity = extract_identity_from_text(
        "## Identification du compte rendu\n"
        "\n"
        "**Noms :** Durand Alice\n"
        "**Groupe :** Binôme 4\n"
        "**Date de la séance :** 22 juin\n"
    )

    assert identity.names == "Durand Alice"
    assert identity.group == "Binôme 4"
    assert identity.week == ""
    assert identity.session_date == "22 juin"
...
def test_split_identity_students_handles_monomes_binomes_trinomes() -> None:
    ...
    assert split_identity_students("Durand Alice ; Martin Bob") == [
        ("DURAND", "Alice"),
        ("MARTIN", "Bob"),
    ]
    assert split_identity_students("Durand Alice ; Martin Bob ; Dupont Claire") == [
        ("DURAND", "Alice"),
        ("MARTIN", "Bob"),
        ("DUPONT", "Claire"),
    ]
    assert split_identity_students("Durand Alice et Martin Bob") == [
        ("DURAND", "Alice"),
        ("MARTIN", "Bob"),
    ]
...
def test_split_identity_names_keeps_backward_compatible_single_value() -> None:
    ...
    assert split_identity_names("Durand Alice ; Martin Bob") == (
        "Durand Alice ; Martin Bob",
        "",
    )
...
def test_build_gradebook_rows_adds_email_from_students_file_without_filling_tp_group(tmp_path: Path) -> None:
    ...
    assert [(row.last_name, row.first_name, row.email, row.group, row.week) for row in rows] == [
        ("DURAND", "Alice", "alice.durand@example.test", "", "25"),
        ("MARTIN", "Bob", "bob.martin@example.test", "", "25"),
    ]
...
def test_build_gradebook_result_reports_unmatched_students(tmp_path: Path) -> None:
    students_file = tmp_path / "students.csv"
    _write_students_csv(students_file)

    _write_notebook(
        tmp_path / "copie-inconnue.ipynb",
        [
            {
                "cell_type": "markdown",
                "metadata": {},
                "source": [
                    "## Identification du compte rendu\n",
                    "\n",
                    "**Noms :** Durand Alice ; Inconnu Test\n",
                    "**Groupe :** Binôme 3\n",
                    "**Semaine :** 25\n",
                ],
            }
    ...
    assert result.rows[0].email == "alice.durand@example.test"
    assert result.rows[1].email == ""

    assert len(result.unmatched_students) == 1
    assert result.unmatched_students[0].entered_last_name == "INCONNU"
    assert result.unmatched_students[0].entered_first_name == "Test"
    assert result.unmatched_students[0].notebook_name == "copie-inconnue.ipynb"
    assert result.unmatched_students[0].identity_names == "Durand Alice ; Inconnu Test"
    assert result.unmatched_students[0].group == "Binôme 3"
    assert result.unmatched_students[0].week == "25"
    assert result.unmatched_students[0].reason == "étudiant non trouvé dans la liste officielle"


def test_export_gradebook_csv_writes_unmatched_report(tmp_path: Path) -> None:
    students_file = tmp_path / "students.csv"
    _write_students_csv(students_file)
    ...
    unmatched_output = tmp_path / "anomalies.csv"
    ...
    with unmatched_output.open(encoding="utf-8", newline="") as stream:
    ...
    assert rows == [
        {
            "Nom saisi": "INCONNU",
            "Prénom saisi": "Test",
            "Nom du notebook": "copie-inconnue.ipynb",
            "Noms cellule": "Inconnu Test",
            "Groupe": "Binôme 3",
            "Semaine de kholle n°": "25",
            "Raison": "étudiant non trouvé dans la liste officielle",
        }
    ]



def test_build_gradebook_result_reports_missing_students(tmp_path: Path) -> None:
    students_file = tmp_path / "students.csv"
    _write_students_csv(students_file)
    ...
    assert [(student.last_name, student.first_name, student.email) for student in result.missing_students] == [
        ("MARTIN", "Bob", "bob.martin@example.test"),
        ("DUPONT", "Claire", "claire.dupont@example.test"),
    ]


def test_export_missing_students_csv(tmp_path: Path) -> None:
    students_file = tmp_path / "students.csv"
    _write_students_csv(students_file)
...
def test_export_missing_students_csv_writes_header_when_empty(tmp_path: Path) -> None:
    output = tmp_path / "copies-manquantes.csv"

    export_missing_students_csv([], output)
...
def test_export_unmatched_students_csv_writes_header_when_empty(tmp_path: Path) -> None:
    output = tmp_path / "anomalies.csv"

    export_unmatched_students_csv([], output)
...
def test_build_gradebook_rows_creates_one_row_per_student(tmp_path: Path) -> None:
    ...
    assert {row.week for row in rows} == {"25"}


def test_build_gradebook_rows_ignores_reference_notebooks_without_identity(tmp_path: Path) -> None:
...
def test_build_gradebook_rows_keeps_student_copy_even_if_ameliore(tmp_path: Path) -> None:
    _write_notebook(
        tmp_path / "Fausse-copie-etudiant-Lois-de-Snell-Descartes-ameliore.ipynb",
        [
            {
                "cell_type": "markdown",
                "metadata": {},
    ...
    assert len(rows) == 1
    assert rows[0].notebook_name == "Fausse-copie-etudiant-Lois-de-Snell-Descartes-ameliore.ipynb"
    assert rows[0].last_name == "DURAND"
    assert rows[0].first_name == "Alice"



def test_build_gradebook_rows_keeps_unknown_student_filename_as_anomaly(tmp_path: Path) -> None:
